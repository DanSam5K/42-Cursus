...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/06 16:37:36 by dsamuel           #+#    #+#             */
/*   Updated: 2024/09/13 13:53:17 by dsamuel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_open_map(char *map, t_game_instance *game_init)
{
	...
	if (fd == -1)
		return (0);
	game_init->map_init.first_read_matrix = ft_read_count_map(map);
	if (!game_init->map_init.first_read_matrix
		|| game_init->map_init.first_read_matrix < 3)
	{
		close (fd);
...
int	ft_read_map(int fd, t_game_instance *game_init)
{
	...
	game_init->map_init.matrix
		= ft_calloc(game_init->map_init.first_read_matrix + 1, sizeof(char *));
	if (!game_init->map_init.matrix)
	{
		ft_free_map(game_init);
		return (0);
	}
	...
	while (1)
	{
		...
		if (!buffer)
			break ;
		game_init->map_init.matrix[i] = buffer;
		i++;
	}
...
{
	if (game_init->map_init.matrix[0] == NULL
